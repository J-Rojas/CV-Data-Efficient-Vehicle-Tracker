...
class TrackingObject():
    ...
    def get_sizes_as_array(self, idxes=None):        
        ...
        min_idx = min(regions)
        max_idx = max(regions)
        arr = np.zeros((max_idx - min_idx + 1, 2))
...
class Tracker():
    ...
    def determine_tracking_trajectories(self) -> tuple[dict[int, dict[int, tuple]], dict[int, dict[int, tuple]]]:
        ...
        for obj in self.objects:
            ...
            sizes, _, _ = obj.get_sizes_as_array() # note: there can be missing frames in this list
            ...
            std_map = windows.std(axis=(-2, -1))
            valid_items = np.all(
                (sizes.T >= sizes_mean - std_map) & (sizes.T <= sizes_mean + std_map),
                axis=0
            )
            ...
            occluded_frames = list(filter(lambda x: x not in obj.regions.keys(), keyframes + min_frame))
            ...
            for idx, _ in obj.frame_pixels.items():
                ...
                if abs(keyframe - idx) <= self.SLIDING_WINDOW_SIZE // 2:
                    ...
                    print(idx, keyframe, y_off, x_off, sizes_mean_kf[keyframe_idx - 1, 0], sizes_mean_kf[keyframe_idx - 1, 1])
                    # reposition and apply the mean width and height
                    cor_bboxes[idx] = (int(bbox[0] - y_off), int(bbox[1] - x_off), int(bbox[2]), int(bbox[3])) # - y_off + sizes_mean_kf[keyframe_idx - 1, 0]), int(bbox[1] - x_off + sizes_mean_kf[keyframe_idx - 1, 1]))                    
                else: 
                    
                    # the object is likely occluded, check if the object exists in both the previous and next frame
                    if obj.regions.get(max(0, idx - 1)) is not None and \
                       obj.regions.get(min(len(obj.regions) - 1, idx + 1)) is not None:
                        
                        # mark the object and frame for coalescing as post-processing step
                        occluded_frames.append(idx)
